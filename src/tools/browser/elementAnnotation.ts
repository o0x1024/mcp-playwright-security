...
/**
 * Tool for taking screenshot with element annotations
 */
export class AnnotatedScreenshotTool extends BrowserToolBase {
  /**
   * Execute annotated screenshot
   */
  async execute(args: any, context: ToolContext): Promise<ToolResponse> {
    return this.safeExecute(context, async (page) => {
      // First annotate elements
      const elements = await page.evaluate(getAnnotationScript()) as AnnotatedElement[];
      ...
      const filename = `${args.name || 'annotated'}-${timestamp}.png`;
      ...
      // Remove annotations after screenshot if requested
      if (args.removeAfter !== false) {
        await page.evaluate(getRemoveAnnotationScript());
      }
      ...
      if (args.savePng !== false) {
        messages.push(`Annotated screenshot saved to: ${path.relative(process.cwd(), outputPath)}`);
      }
      messages.push(`Found ${elements.length} interactive elements`);
